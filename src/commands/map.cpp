...
namespace Commands
{
	...
	void HideImpl(const std::vector<std::string> &arguments, Character *from, bool set)
	{
		...
		{
			if (flag == "invisible")
				flags |= Character::HideInvisible;
			else if (flag == "online")
				flags |= Character::HideOnline;
			else if (flag == "npc")
				flags |= Character::HideNpc;
			else if (flag == "admin")
				flags |= Character::HideAdmin;
			else if (flag == "warp")
				flags |= Character::HideWarp;
			else if (flag == "all")
				flags |= Character::HideAll;
			else
			{
