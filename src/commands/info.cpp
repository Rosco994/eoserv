	...
	void Inventory(const std::vector<std::string> &arguments, Character *from)
	{
		...
		else
		{
			if (victim->SourceAccess() < int(from->world->admin_config["cmdprotect"]) || victim->SourceAccess() <= from->SourceAccess())
			{
				...
				{
				case ADMIN_HGM:
					reply.AddString(from->world->i18n.Format("high_game_master_title", name));
					break;
				case ADMIN_GM:
					reply.AddString(from->world->i18n.Format("game_master_title", name));
					break;
				case ADMIN_GUARDIAN:
					reply.AddString(from->world->i18n.Format("guardian_title", name));
					break;
				case ADMIN_GUIDE:
					reply.AddString(from->world->i18n.Format("light_guide_title", name));
					break;
				default:
	...
	void Paperdoll(const std::vector<std::string> &arguments, Character *from)
	{
		...
		else
		{
			...
			PacketBuilder reply(PACKET_PAPERDOLL, PACKET_REPLY,
								12 + victim->SourceName().length() + home_str.length() + victim->partner.length() + victim->title.length() + guild_str.length() + rank_str.length() + victim->paperdoll.size() * 2);
	...
	void Book(const std::vector<std::string> &arguments, Character *from)
	{
		...
		else
		{
			...
			PacketBuilder reply(PACKET_BOOK, PACKET_REPLY,
								13 + victim->SourceName().length() + home_str.length() + victim->partner.length() + victim->title.length() + guild_str.length() + rank_str.length());
