/* player_commands/player_commands.hpp
 * EOSERV is released under the zlib license.
 * See LICENSE.txt for more info.
 */

#ifndef PLAYER_COMMANDS_HPP_INCLUDED
#define PLAYER_COMMANDS_HPP_INCLUDED
...
#define PLAYER_COMMAND_HANDLER_REGISTER(uid)                                                                                           \
	namespace                                                                                                                          \
	{                                                                                                                                  \
		struct PLAYER_COMMAND_HANDLER_PASTE_AUX(command_handler_register_helper_, uid) : public player_command_handler_register_helper \
		{                                                                                                                              \
			PLAYER_COMMAND_HANDLER_PASTE_AUX(command_handler_register_helper_, uid)()                                                  \
			{                                                                                                                          \
				command_handler_register_init _init;

#define PLAYER_COMMAND_HANDLER_REGISTER_END(uid)                                      \
	;                                                                                 \
	}                                                                                 \
	}                                                                                 \
	PLAYER_COMMAND_HANDLER_PASTE_AUX(command_handler_register_helper_instance_, uid); \
	}
...
namespace PlayerCommands
{
	...
	struct command_info
	{
		...
		command_info(std::string name, empty_vector_init<std::string> arguments = empty_vector_init<std::string>(),
			...
			: name(name), arguments(arguments), optional_arguments(optional_arguments), partial_min_chars(partial_min_chars), require_character(false)
		{
		}
	};
	...
	struct command_handler
	{
		...
		command_handler(command_info info, std::function<void(const std::vector<std::string> &, Character *)> f)
			: info(info), f([f](const std::vector<std::string> &arguments, Command_Source *from)
							{ f(arguments, static_cast<Character *>(from)); })
		{
	...
	public:
		...
		template <class F>
		void RegisterCharacter(command_info info, const F &f, int flags = 0)
		{
...
#endif // PLAYER_COMMANDS_HPP_INCLUDED
