/* player_commands/player_commands.cpp
...
namespace PlayerCommands
{
	...
	bool command_handler_register::Handle(std::string command, const std::vector<std::string> &arguments, Command_Source *from, int alias_depth) const
	{
		...
		{
			if (command_result->second.info.require_character && !from->SourceCharacter())
			{
				from->ServerMsg(from->SourceWorld()->i18n.Format("unknown_command"));
				return false;
			}
		...
		else
		{
			...
			{
				if (command.length() < handler->second.info.partial_min_chars || command.length() >= handler->second.info.name.length())
					continue;
