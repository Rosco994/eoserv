...
namespace Handlers
{
	...
	void Priest_Request(Character *character, PacketReader &reader)
	{
		...
		if (character->npc_type == ENF::Priest)
		{
			...
			if ((character->gender == GENDER_MALE && male_outfit != 0 && armor != male_outfit) || (character->gender == GENDER_FEMALE && female_outfit != 0 && armor != female_outfit))
			{
				PacketBuilder reply(PACKET_PRIEST, PACKET_REPLY, 2);
			...
			if ((partner->gender == GENDER_MALE && male_outfit != 0 && partner_armor != male_outfit) || (partner->gender == GENDER_FEMALE && female_outfit != 0 && partner_armor != female_outfit))
			{
				PacketBuilder reply(PACKET_PRIEST, PACKET_REPLY, 2);
