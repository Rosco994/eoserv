...
namespace Handlers
{
	...
	void Jukebox_Msg(Character *character, PacketReader &reader)
	{
		...
		if (!character->jukebox_open || character->map->jukebox_protect > Timer::GetTime() || (track < 0 || track > static_cast<int>(character->world->config["JukeboxSongs"])) || character->HasItem(1) < static_cast<int>(character->world->config["JukeboxPrice"]))
		{
			return;
		}
	...
	void Jukebox_Use(Character *character, PacketReader &reader)
	{
		...
		auto find_it = find_if(UTIL_RANGE(character->spells),
							   [&](const Character_Spell &spell)
							   { return esf->Get(spell.id).type == ESF::Bard; });
