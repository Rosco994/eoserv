...
namespace Handlers
{
	...
	void Welcome_Request(Player *player, PacketReader &reader)
	{
		...
		PacketBuilder reply(PACKET_WELCOME, PACKET_REPLY,
							114 + player->character->paperdoll.size() * 2 + player->character->SourceName().length() + player->character->title.length() + guild_str.length() + guild_rank.length());
		...
		int leader_rank = std::max(std::max(std::max(static_cast<int>(player->world->config["GuildEditRank"]), static_cast<int>(player->world->config["GuildKickRank"])),
											static_cast<int>(player->world->config["GuildPromoteRank"])),
								   static_cast<int>(player->world->config["GuildDemoteRank"]));
	...
	void Welcome_Msg(Player *player, PacketReader &reader)
	{
		...
		reply.ReserveMore(7 + player->character->inventory.size() * 6 + player->character->spells.size() * 4 + updatecharacters.size() * 60 + updatenpcs.size() * 6 + updateitems.size() * 9);
	...
	void Welcome_Agree(Player *player, PacketReader &reader)
	{
		...
		{
		case FILE_MAP:
			result = player->client->Upload(FILE_MAP, player->character->mapid, INIT_FILE_MAP);
			break;
		case FILE_ITEM:
			result = player->client->Upload(FILE_ITEM, player->client->next_eif_id++, INIT_FILE_EIF);
			break;
		case FILE_NPC:
			result = player->client->Upload(FILE_NPC, player->client->next_enf_id++, INIT_FILE_ENF);
			break;
		case FILE_SPELL:
			result = player->client->Upload(FILE_SPELL, player->client->next_esf_id++, INIT_FILE_ESF);
			break;
		case FILE_CLASS:
			result = player->client->Upload(FILE_CLASS, player->client->next_ecf_id++, INIT_FILE_ECF);
			break;
		default:
			return;
		}
